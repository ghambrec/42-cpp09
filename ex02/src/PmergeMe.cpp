...
// a = bigger value
static void fjv_build_and_sort_pairs(std::vector<int>& v, size_t count_pairs, size_t element_size)
{
...
// main: b1, a1, a2, a3, ...
// pend: b2, b3, b4, ....
static void fjv_build_chains(std::vector<int>& v, std::vector<int>& main, std::vector<int>& pend, size_t element_size, size_t count_elements, size_t count_pairs)
{
	// for each pair
	for (size_t pair = 0; pair < count_pairs; pair++)
...
static void fjv_insert_pend_in_main(std::vector<int>& main, std::vector<int>& pend, size_t element_size, size_t count_elements, size_t count_pairs)
{
...
void fjv_algo(std::vector<int>& v, size_t element_size)
{
	...
	// step 1
	fjv_build_and_sort_pairs(v, count_pairs, element_size);
	fjv_algo(v, element_size * 2);
	...
	// step 2
	fjv_build_chains(v, main, pend, element_size, count_elements, count_pairs);

	// step 3
	fjv_insert_pend_in_main(main, pend, element_size, count_elements, count_pairs);
...
// b = smaller value
// a = bigger value
static void fjd_build_and_sort_pairs(std::deque<int>& d, size_t count_pairs, size_t element_size)
{
	for (size_t pair = 0; pair < count_pairs; pair++)
	{
		size_t start_left = (2 * pair) * element_size;
		...
		if (d[key_left] > d[key_right]) // vergleich
		{
			for (size_t i = 0; i < element_size; i++)
			{
				std::swap(d[start_left + i], d[start_right + i]);
			}
		}
	}
}
...
// main: b1, a1, a2, a3, ...
// pend: b2, b3, b4, ....
static void fjd_build_chains(std::deque<int>& d, std::deque<int>& main, std::deque<int>& pend, size_t element_size, size_t count_elements, size_t count_pairs)
{
	// for each pair
	for (size_t pair = 0; pair < count_pairs; pair++)
	{
		...
		// b
		if (pair == 0) // b1 to main chain
			main.insert(main.end(), d.begin() + start_left, d.begin() + start_left + element_size);
		else // other b's in pend chain
			pend.insert(pend.end(), d.begin() + start_left, d.begin() + start_left + element_size);

		// a to main chain
		main.insert(main.end(), d.begin() + start_right, d.begin() + start_right + element_size);
	}
	// add unpaired element to pend
	if (count_elements % 2 == 1)
	{
		size_t start_missing = (count_elements - 1) * element_size;
		pend.insert(pend.end(), d.begin() + start_missing, d.begin() + start_missing + element_size);
	}
}

// STEP 3 - INSERT PEND IN MAIN CHAIN
// respect the border: b2 has to be before a2
static void fjd_insert_pend_in_main(std::deque<int>& main, std::deque<int>& pend, size_t element_size, size_t count_elements, size_t count_pairs)
{
	size_t pend_count = pend.size() / element_size;
	if (pend_count > 0)
	{
		...
		// save original position in main for border
		std::deque<size_t> main_positions;
		for (size_t i = 0; i <= count_pairs; i++)
			main_positions.push_back(i); // b1, a1, a2, a3, ...
		...
		for (size_t i = 0; i < insertion_order.size(); i++)
		{
			...
			// get current pend element
			std::deque<int> cur_pend_element(pend.begin() + pend_idx * element_size, pend.begin() + (pend_idx + 1) * element_size);
			int key = cur_pend_element[element_size - 1];
...
{
	if ((d.size() / element_size) < 2)
		return;

	// calculate base params
	size_t count_elements = d.size() / element_size;
	size_t count_pairs = count_elements / 2;

	// step 1
	fjd_build_and_sort_pairs(d, count_pairs, element_size);
	fjd_algo(d, element_size * 2);
	...
	// step 2
	fjd_build_chains(d, main, pend, element_size, count_elements, count_pairs);

	// step 3
	fjd_insert_pend_in_main(main, pend, element_size, count_elements, count_pairs);
